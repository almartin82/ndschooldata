# ==============================================================================
# LIVE Pipeline Tests for ndschooldata
# ==============================================================================
#
...
test_that("North Dakota DOE website is accessible", {

  skip_if_offline()
  skip("TODO: Add North Dakota DOE URL and verify HTTP 200")
})
...
test_that("Can download North Dakota enrollment data file", {
  skip_if_offline()
...
test_that("Can parse North Dakota enrollment file", {
  skip_if_offline()
...
test_that("ndschooldata data file has expected columns", {
...
test_that("get_raw_enr returns data for valid year", {
  ...
  # Get available years
  years_info <- ndschooldata::get_available_years()
  ...
  tryCatch({
    raw <- ndschooldata:::get_raw_enr(test_year)
...
test_that("get_available_years returns valid year range", {
  result <- ndschooldata::get_available_years()
...
test_that("fetch_enr returns data with no Inf or NaN", {
  ...
  tryCatch({
    ...
    data <- ndschooldata::fetch_enr(test_year, tidy = TRUE)
...
test_that("Enrollment counts are non-negative", {
  ...
  tryCatch({
    ...
    data <- ndschooldata::fetch_enr(test_year, tidy = FALSE)
...
test_that("tidy=TRUE and tidy=FALSE return consistent totals", {
  ...
  tryCatch({
    ...
    wide <- ndschooldata::fetch_enr(test_year, tidy = FALSE)
    tidy <- ndschooldata::fetch_enr(test_year, tidy = TRUE)
...
test_that("Cache functions exist and work", {
  ...
  tryCatch({
    path <- ndschooldata:::get_cache_path(2024, "enrollment")
